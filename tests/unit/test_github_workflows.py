...
class TestWorkflowStructure:
    ...
    def workflows_dir(self):
        """
        Locate the repository's GitHub Actions workflows directory.
        
        Returns:
            workflows_dir (Path): Path to the repository's .github/workflows directory.
        """
    ...
    def manual_workflow(self, workflows_dir):
        """
        Load and parse the manual GitHub Actions workflow file (manual.yml).
        
        Parameters:
            workflows_dir (Path): Path to the .github/workflows directory containing manual.yml.
        
        Returns:
            dict: Parsed YAML content of manual.yml.
        """
    ...
    def test_manual_workflow_has_permissions(self, manual_workflow):
        """
        Ensure the workflow declares a permissions section with the `contents` permission set to "read".
        
        Asserts that a top-level `permissions` mapping exists and that `permissions['contents']` equals `"read"`, enforcing read-only access to repository contents.
        """
    ...
    def test_manual_workflow_inputs(self, manual_workflow):
        """
        Validate that the workflow's `workflow_dispatch` inputs include a properly configured `name` input.
        
        Asserts that `workflow_dispatch.inputs.name` exists and has:
    ...
    def test_manual_workflow_greeting_step(self, manual_workflow):
        """
        Validate the 'Send greeting' step in the 'greet' job of the workflow.
        
        Asserts that a step named 'Send greeting' exists, contains a 'run' command that uses `echo`, includes the literal "Hello", and references `${{ inputs.name }}`.
        
        Parameters:
            manual_workflow (dict): Parsed YAML mapping of the manual.yml workflow.
        """
...
class TestWorkflowBestPractices:
    ...
    def test_workflow_naming_conventions(self, manual_workflow):
        """
        Validate that each job name is alphanumeric (ignoring '-' and '_') and at least three characters long.
        
        Asserts that job names contain only letters and digits once hyphens and underscores are removed, and that each name has length >= 3 to ensure descriptiveness.
        """
    ...
    def test_workflow_runner_valid(self, manual_workflow):
        """
        Validate that the 'greet' job's runner is an allowed hosted runner or a self-hosted runner.
        
        Asserts that the `runs-on` value for the `greet` job contains one of the permitted hosted runner identifiers (e.g., `ubuntu-latest`, `windows-latest`, `macos-latest`) or starts with `self-hosted`.
        """
...
class TestWorkflowSecurity:
    ...
    def test_input_validation(self, manual_workflow):
        """Test that workflow considers input validation"""
        on_config = manual_workflow.get('on', manual_workflow.get(True, {}))
        inputs = on_config['workflow_dispatch']['inputs']
    ...
    def test_no_hardcoded_secrets(self, manual_workflow):
        """
        Fail the test if the parsed workflow YAML contains likely hard-coded secret keys.
        
        Scans the serialized workflow for common secret-identifying prefixes (for example `password:`, `token:`, `api_key:`, `secret:`, `credential:`) and fails if any occurrence is found outside of templated or referenced secrets (e.g., `{{ ... }}` or `secrets.`).
        
        Parameters:
            manual_workflow (dict): Parsed YAML content of the workflow (from yaml.safe_load).
        """
...
class TestWorkflowEdgeCases:
    ...
    def test_special_characters_handling(self, manual_workflow):
        """
        Ensure the workflow_dispatch 'name' input is declared as a string to allow special characters.
        
        Verifies that the workflow trigger inputs include a 'name' input whose 'type' is 'string'.
        """
        on_config = manual_workflow.get('on', manual_workflow.get(True, {}))
        inputs = on_config['workflow_dispatch']['inputs']
    ...
    def test_required_with_default(self, manual_workflow):
        """
        Ensure the workflow's 'name' workflow_dispatch input provides a default when marked required.
        
        If the 'name' input under `workflow_dispatch.inputs` has `required: True`, this test asserts that a `default` key is present to improve user experience.
        """
        on_config = manual_workflow.get('on', manual_workflow.get(True, {}))
