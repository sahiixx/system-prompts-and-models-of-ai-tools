...
class TestWorkflowStructure:
    ...
    def workflows_dir(self):
        """
        Return the repository's GitHub Actions workflows directory path.
    ...
    def manual_workflow(self, workflows_dir):
        """
        Load and parse the 'manual.yml' workflow file from the workflows directory.
        
        Parameters:
            workflows_dir (pathlike): Path to the directory containing workflow YAML files (e.g., .github/workflows).
        
        Returns:
            dict: Parsed YAML structure representing the workflow (typically a mapping of workflow keys and values).
        """
    ...
    def test_manual_workflow_exists(self, workflows_dir):
        """
        Check that .github/workflows/manual.yml exists and is a regular file.
        
        Parameters:
            workflows_dir (pathlib.Path): Path to the repository's .github/workflows directory.
        """
    ...
    def test_manual_workflow_has_name(self, manual_workflow):
        ...
        Asserts the workflow mapping contains a 'name' key, that its value is a string, and that the string is not empty.
        """
    ...
    def test_manual_workflow_inputs(self, manual_workflow):
        """
        Validate the workflow's `workflow_dispatch` `name` input configuration.
        
        Asserts that `workflow_dispatch.inputs.name` exists and that its fields match expectations:
        - `description` is "Person to greet"
        - `default` is "World"
        - `required` is `True`
        - `type` is "string"
        """
    ...
    def test_manual_workflow_greeting_step(self, manual_workflow):
        """Test the greeting step configuration"""
...
class TestWorkflowBestPractices:
    ...
    def test_workflow_runner_valid(self, manual_workflow):
        ...
        Asserts the `runs-on` value for `jobs.greet` contains one of the permitted hosted runner identifiers (e.g., ubuntu-latest, windows-latest) or begins with `self-hosted`.
        """
...
class TestWorkflowSecurity:
    ...
    def test_least_privilege(self, manual_workflow):
        """
        Ensure the workflow does not request write permissions in its `permissions` mapping.
        """
        permissions = manual_workflow.get('permissions', {})
    ...
    def test_input_validation(self, manual_workflow):
        ...
        Checks the parsed workflow YAML and asserts that every input marked as required under `on.workflow_dispatch.inputs`
        includes a `type` field; raises an assertion error identifying the offending input when absent.
        
        Parameters:
            manual_workflow (dict): Parsed workflow YAML mapping (typically the contents of .github/workflows/manual.yml).
        """
        on_config = manual_workflow.get('on', manual_workflow.get(True, {}))
        inputs = on_config['workflow_dispatch']['inputs']
...
class TestWorkflowEdgeCases:
    ...
    def test_yaml_indentation(self, workflows_dir):
        """
        Checks that indentation for non-empty, non-comment lines in manual.yml is a multiple of two spaces.
        
        This verifies YAML indentation consistency by examining each non-empty, non-comment line and ensuring its leading space count is divisible by 2; the test fails if any line violates this rule.
        """
        with open(workflows_dir / 'manual.yml', 'r') as f:
            content = f.read()
        lines = content.split('\n')
    ...
    def test_ends_with_newline(self, workflows_dir):
        """
        Assert that 'manual.yml' ends with a newline character.
        """
