...
class TestWorkflowStructure:
    ...
    def workflows_dir(self):
        ...
        Returns:
            Path: Path object pointing to the `.github/workflows` directory at the repository root.
        """
    ...
    def manual_workflow(self, workflows_dir):
        """
        Load and parse the repository's manual GitHub Actions workflow file at `.github/workflows/manual.yml`.
        
        Parameters:
            workflows_dir (pathlib.Path): Path to the `.github/workflows` directory.
        
        Returns:
            dict | list: Parsed YAML content of the workflow (typically a mapping/dict).
        """
    ...
    def test_manual_workflow_inputs(self, manual_workflow):
        """
        Verify the workflow defines a `workflow_dispatch` input named `name` with the expected metadata.
        
        Asserts that `workflow_dispatch.inputs` contains a `name` entry whose `description` is "Person to greet", `default` is "World", `required` is `True`, and `type` is "string".
        """
        on_config = manual_workflow.get('on', manual_workflow.get(True, {}))
    ...
    def test_manual_workflow_greeting_step(self, manual_workflow):
        """
        Verify the 'greet' job contains a 'Send greeting' step that uses a run command to echo the text "Hello" and references the workflow input name via `${{ inputs.name }}`.
        """
...
class TestWorkflowBestPractices:
    ...
    def test_workflow_runner_valid(self, manual_workflow):
        """
        Ensure the job's 'runs-on' value is an allowed GitHub runner or a self-hosted pattern.
        
        Verifies that the `greet` job's `runs-on` contains one of: `ubuntu-latest`, `ubuntu-22.04`, `ubuntu-20.04`, `windows-latest`, `macos-latest`, or starts with `self-hosted`.
        """
...
class TestWorkflowSecurity:
    ...
    def test_least_privilege(self, manual_workflow):
        """
        Verify the workflow's permissions do not include any entries set to 'write'.
        
        Asserts that no permission key in the workflow's `permissions` mapping has the value `'write'`.
        """
        permissions = manual_workflow.get('permissions', {})
    ...
    def test_input_validation(self, manual_workflow):
        """
        Verify that any required workflow_dispatch inputs declare a `type`.
        
        This test inspects the workflow's `workflow_dispatch.inputs` mapping and asserts that each input marked `required: true` also includes a `type` field; if not, the test fails with a message naming the offending input.
        
        Parameters:
            manual_workflow (dict): Parsed YAML mapping of the workflow file.
        """
        on_config = manual_workflow.get('on', manual_workflow.get(True, {}))
        inputs = on_config['workflow_dispatch']['inputs']
...
class TestWorkflowEdgeCases:
    ...
    def test_yaml_indentation(self, workflows_dir):
        """
        Validate that non-empty, non-comment lines in .github/workflows/manual.yml use indentation in multiples of two spaces.
        
        Reads the workflow file at workflows_dir / 'manual.yml', ignores blank lines and lines starting with '#', measures leading-space indentation for remaining lines, and fails if any measured indentation is not a multiple of two.
